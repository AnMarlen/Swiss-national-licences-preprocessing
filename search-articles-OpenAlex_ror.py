...
ror_id = config.get("Institution", "ror_id")
...
def build_zhaw_works_url(ror_id, publishers):
...
while has_more_pages and fewer_than_10k_results:
    ...
    example_url_with_page = build_zhaw_works_url(ror_id, publishers)
    url = example_url_with_page.format(page)
    page_with_results = requests.get(url).json()

    # loop through list of results and write selected elements in dataframe
...
df.to_csv(f"{path}{file_ror}", index=False, encoding="utf-8")
print(f"{file_ror} saved in path: {path}")
